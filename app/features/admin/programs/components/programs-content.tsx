...
export function ProgramsContent({ programs, actionData }: ProgramsContentProps) {
  ...
  return (
    <div className="h-full bg-transparent relative overflow-auto" style={{ fontFamily: 'Pretendard, Inter, sans-serif' }}>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8 relative z-10 pt-4 md:pt-14 lg:pt-[4.5rem]">
        {actionData?.success && (
          ...
          <div className="px-4 py-4 sm:px-6 sm:py-5">
            <div className="flex items-center gap-3 sm:gap-4">
              <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-2xl flex items-center justify-center shadow-lg bg-white/30 backdrop-blur-sm flex-shrink-0">
                <FolderOpen className="w-5 h-5 sm:w-7 sm:h-7 text-white drop-shadow-sm" />
              </div>
              <div className="min-w-0 flex-1">
                <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-white drop-shadow-sm mb-1" style={{ lineHeight: '1.6' }}>프로젝트 관리</h1>
                <p className="text-white/90 flex items-center gap-2 text-xs sm:text-sm" style={{ lineHeight: '1.6' }}>
                  <Sparkles className="w-3 h-3 sm:w-4 sm:h-4 flex-shrink-0" />
                  <span className="truncate">프로젝트 콘텐츠 및 공개 상태를 관리하세요</span>
                </p>
              </div>
            </div>
          </div>
        ...
        {programs.length === 0 ? (
          <Card className="border border-gray-200 shadow-sm bg-white">
            <CardContent className="p-8 sm:p-16 text-center">
              ...
              <p className="text-[#3B2F2F] text-base sm:text-lg font-extrabold tracking-tight" style={{ lineHeight: '1.6' }}>등록된 프로그램이 없습니다.</p>
              <p className="text-[#7A6666] text-xs sm:text-sm mt-2 opacity-80" style={{ lineHeight: '1.6' }}>프로그램을 추가해보세요</p>
            </CardContent>
          </Card>
        ) : (
            ...
            {programs.map((p) => {
              ...
              return (
                <Card key={p.id} className="overflow-hidden border border-gray-200 shadow-sm bg-white hover:shadow-md transition-all duration-300 group">
                  <CardHeader className="border-b border-gray-100 pb-3 sm:pb-4 relative overflow-hidden bg-gray-50 p-4 sm:p-6">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 relative z-10">
                      <div className="flex items-center gap-3 sm:gap-4 min-w-0 flex-1">
                        {p.icon && (
                          <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl flex items-center justify-center text-2xl sm:text-3xl shadow-lg group-hover:scale-110 group-hover:rotate-3 transition-all duration-300 flex-shrink-0" style={{ background: 'linear-gradient(90deg, #F3C3E6, #FFE6C5)' }}>
                            {p.icon}
                          </div>
                        )}
                        <div className="min-w-0 flex-1">
                          <CardTitle className="text-xl sm:text-2xl font-extrabold tracking-tight text-[#3B2F2F] mb-2" style={{ lineHeight: '1.6' }}>{p.title || "이름 없음"}</CardTitle>
                      ...
                      <Form method="post" className="w-full sm:w-auto">
                        ...
                        <Button 
                          type="submit" 
                          size="sm"
                          className={`w-full sm:w-auto shadow-md hover:shadow-lg transition-all text-white ${
                            p.is_active 
